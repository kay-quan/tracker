#!/usr/bin/env python3
"""Build public/artists.js from the Life Tracker kit's artists.json.

The database is reference data: identical for every user, never edited from the app.
It ships as a static file rather than in Firestore because it is 1.26 MB and a
Firestore document caps at 1 MB -- it physically cannot live in synced state.
...
body = (
    "/* Artist contact database -- generated by tools/gen_artists_js.py. Do not hand-edit.\n"
    "   Source: life-tracker-kit/data/artists.json (Jay Matsumoto). Every address carries\n"
    "   the URL it was read from; nothing here was constructed from a name pattern.\n"
    "   Static, not synced: 1.26 MB of reference data cannot fit a 1 MB Firestore doc. */\n"
    "window.ARTIST_ACTS=" + j(acts) + ";\n"
    "window.ARTIST_LOOKUP=" + j(lookup) + ";\n"
    "window.ARTIST_FESTIVALS=" + j(fests) + ";\n"
    "window.ARTIST_STATS=" + j({"acts": len(real), "reachable": reachable,
                                "addresses": addresses, "leads": leadonly,
                                "festivals": len(fests)}) + ";\n"
)

# The file the app actually imports. This site is served from a PUBLIC repo, so the
# database is NOT shipped as a script next to the page -- it goes into Firestore, behind
# the account's uid rule, via Settings -> Import artist database.
...
if APPLY:
    ...
    print("\nWROTE %s  (%.1f KB)  -- local/dev only, gitignored" % (OUT, len(body) / 1024))
    print("WROTE %s  (%.1f KB)  -- import this in Settings" % (PAYLOAD, len(payload) / 1024))
    if len(payload) > 1000000:
        print("\n!! %.0f KB exceeds the 1 MB Firestore document limit -- it must be split."
              % (len(payload) / 1024))
else:
    print("\nDRY RUN -- would write:")
    print("  %s  (%.1f KB)" % (OUT, len(body) / 1024))
    print("  %s  (%.1f KB)" % (PAYLOAD, len(payload) / 1024))
